...
infl_standard <-
  subset(infl_standard, V1 %in%  rownames(standard_annon))
infl_standard$V3 <-
  ordered(infl_standard$V3,
          levels = c("iPSC", "DE", "PGT", "PFG", "PE", "EP", "EN", "BLC"))
infl_standard <- infl_standard %>% mutate(V1 = plyr::mapvalues(
  V1,
  from = rownames(standard_annon),
  to = as.character(standard_annon$annotation)
))
